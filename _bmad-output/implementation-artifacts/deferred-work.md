...
- source_spec: `_bmad-output/specs/spec-charges/SPEC.md` (ревью волны 3, подтверждённое чужое)
  summary: Пять подтверждённых дефектов вне моих файлов. (1) Длина `codeAnswer` не сверяется ни с уроком, ни со списком показанного: `codeAnswer: "9"` при `percentScore: 100` проходит `scoreVerified` и приносит полную награду за двадцативопросный урок и стопроцентную турнирную запись. (2) `calculateTournamentLeaderboard` сортирует рейтинги из несвязанных компонент в одной таблице, поэтому второй аккаунт занимает первое место. (3) `completedAtMs` — неограниченный клиентский ввод, и он выбирает турнирную группу. (4) `upsertUserProfile` пишет `hasPremium: … || Boolean(existingUser.hasPremium)`, то есть однажды выданный премиум не истекает. (5) Раннер перечитывает число зарядов из профиля на каждой попытке, поэтому честная офлайн-игра упирается в аудит перерасхода.
