...
- source_spec: `_bmad-output/planning-artifacts/epics-sync.md` (Story 10.3, Open Questions)
  summary: Чем на самом деле должно быть событие `qualification.AUTHOR_RATING_CHANGED` — вопрос владельцу; «слать при смене уровня» сегодня не к чему привязать.
  evidence: Разбор 2026-09-02: уровня квалификации по авторскому рейтингу в коде НЕТ. `authorRatingScore` — репутация, и ADR-0006 отделяет её от шести квалификаций с порогом 100; комментарий в `writeAuthorRatingScoreDelta` прямо говорит, что размазывание этого числа по картам квалификаций и было прежней ошибкой. Событие, обещанное спайном («оффер квалификации»), серверной сущности не имеет вовсе. Сделано частично: уведомление уходит только нынешнему автору и только когда число выросло (`writeRatingQualificationDelta`, страж `rating-push-recipients` в `push-notifications.test.js`) — прежний владелец при переносе рейтинга и падение числа больше не уведомляются. Осталось решение владельца: либо завести пороги для рейтинга автора и слать на пересечении, либо снять это событие до появления оффера квалификации, оставив два реальных (квест опубликован, верификация одобрена).
...
- source_spec: `buildSrc/src/main/kotlin/KmpLibraryConventionPlugin.kt` (найдено при сборке эпиков 6–7)
  summary: Статический анализ и форматирование не смотрят на `shared/**` вообще — весь общий код проекта вне гейта.
  evidence: `:shared:*:detekt` — `NO-SOURCE` (исходники KMP лежат в `commonMain`/`jvmMain`, а не в `src/main/java`), и именно эту задачу собирают корневой агрегат и `ciCheck`; ktlint в KMP-модулях не заводится ни одной задачей (`:shared:core:outbox:tasks --all` — ноль совпадений на ktlint). Настоящий анализ даёт `detektMetadataCommonMain`: прогон по всем 48 модулям `shared/**` — **207 нарушений** (MagicNumber 120, TooGenericExceptionCaught 33, ReturnCount 23, MaxLineLength 12, TooManyFunctions 7, прочее 12). То есть ядро проекта — очередь, синхронизация, персистентность, все домены и data-слои фич — не проверялось ни разу. Починка: в `KmpLibraryConventionPlugin` применить ktlint и повесить `check` на `detektMetadata*`; следом разобрать 207 существующих нарушений (большинство — MagicNumber, часть снимается подавлением по месту с объяснением). Это отдельная работа: правка гейта без разбора нарушений сделает сборку красной.
  note: Тот же класс дыры, что и «androidTest никогда не компилировался» (найдено в эпике 3): зелёный гейт означал не «чисто», а «не смотрели».

- source_spec: `_bmad-output/specs/spec-charges/SPEC.md` (ревью волны 3, подтверждённое чужое)
  summary: Пять подтверждённых дефектов вне моих файлов. (1) ЗАКРЫТО: приёмник переключён на `readSubmittedAttempt`, оплата идёт по `isPayable`, и число показанных сверяется с набором урока. (2) `calculateTournamentLeaderboard` сортирует рейтинги из несвязанных компонент в одной таблице, поэтому второй аккаунт занимает первое место. (3) `completedAtMs` — неограниченный клиентский ввод, и он выбирает турнирную группу. (4) `upsertUserProfile` пишет `hasPremium: … || Boolean(existingUser.hasPremium)`, то есть однажды выданный премиум не истекает. (5) Раннер перечитывает число зарядов из профиля на каждой попытке, поэтому честная офлайн-игра упирается в аудит перерасхода.
  evidence: `functions/index.js` `normalizeLessonResultAttemptEvent` (нет ни длины, ни `served`; противоядие `servedMatchesDigits` в `attempt-intake.js` существует, но `index.js` этот модуль не подключает); `functions/tournament-ranking.js` `solveComponentRatings` + `compareLeaderboardEntries`; `functions/index.js` `tournamentGroupForAttempt(event)` по `event.completedAtMs`; `functions/index.js` строка `hasPremium:` в `upsertUserProfile`; `DefaultLessonRunnerRootComponent.readLivesFromProfile`. Первое и второе — блокеры. Первое закрывается подключением `attempt-intake.js`, которое ведёт соседняя сессия; пятое — это заблокированная история 8.
- source_spec: `_bmad-output/planning-artifacts/epics-sync.md` (Story 5.3, обзор ветки 2026-09-03)
  summary: Прямой путь разблокировки урока остался в economy как мёртвый код: `EconomyRepositoryImpl.unlockLesson` и `FirebaseEconomyRemoteDataSource.unlockLesson` живы, хотя прод-путь целиком идёт очередью.
  evidence: Поиск по `android`, `apps`, `shared/feature/lesson` не находит ни одного вызова — экран зовёт `LessonUnlockQueue.queueUnlock`. Файлы принадлежат сессии монетизации, поэтому не тронуты. По AD-6 у отложенной мутации один транспорт; второй путь — приглашение обойти ключ идемпотентности, как это уже случилось с четырьмя callable ревью и полок. Снять вместе с серверным callable `unlockLesson`, когда монетизация освободит файлы.
- source_spec: `_bmad-output/planning-artifacts/epics-sync.md` (Story 5.3, решение владельца 2026-09-03)
  summary: Тело записи разблокировки несёт третье поле — списанную сумму, — которого нет в критерии истории.
  evidence: Критерий говорит «payload несёт lessonId и kind». Поле `chargedNolics` добавлено, чтобы откат при карантине вернул ровно списанное: пересчёт цены через дни дал бы другое число (вопросы урока успевают измениться), и возврат разошёлся бы со списанием. Сервер поле не читает и цену считает сам, так что AD-3 не нарушен. Либо признать отклонение в тексте истории, либо хранить число вне тела — это уже схема.

- source_spec: `_bmad-output/implementation-artifacts/spec-m1-2-client-settles-after-confirmation.md`
  summary: The purchase fakes are duplicated between the economy domain and data test source sets, and they will drift.
  evidence: `shared/feature/economy/data/src/commonTest/.../UnsettledPurchaseSettlerTest.kt` re-implements `FakeBillingRepository`, `FakePurchaseVerifier`, `FakeNetworkMonitor` and `FakeServerBalanceRefresher` from `shared/feature/economy/domain/src/commonTest/.../fake/PurchaseFakes.kt`, down to the backlog comments. `.claude/rules/testing.md` forbids duplicate fakes; the repo has no test-fixtures source set anywhere, so the data module genuinely cannot see the domain module's `commonTest` today. The two copies assert settlement ordering — the place where drift costs money. Fix: one `economy:test-fixtures` module (or a `commonTestFixtures` source set) holding `PurchaseFakes.kt`.
- source_spec: `_bmad-output/implementation-artifacts/spec-m1-2-client-settles-after-confirmation.md`
  summary: A token the server refuses for this account is never handed back to the store, so that pack can never be bought again on that device.
  evidence: Paid under account A, never consumed, device signs in as B. The server refuses `TOKEN_OWNED_BY_ANOTHER_ACCOUNT`, the settler remembers the refusal and never consumes, and Play then answers `ITEM_ALREADY_OWNED` for every future purchase of that SKU by B — which `BuyGoldPackUseCase` settles into another refusal. Deciding whether a refused token may be consumed is a product call the M1.2 Design Notes deliberately deferred; it needs an answer before the shop ships (story 1.4).
- source_spec: `_bmad-output/implementation-artifacts/spec-m1-3-refund-handler.md`
  summary: A clawed-back balance leaves a debt the device never shows.
  evidence: `goldShortfall` is written by the refund handler and is not part of `readEconomyBalance`, so a player whose gold was taken back below zero sees an unexplained balance until the receipt screen exists (story 5.4). Deliberate — story 1.3 adds no client code — but it must not ship to players in that state.
