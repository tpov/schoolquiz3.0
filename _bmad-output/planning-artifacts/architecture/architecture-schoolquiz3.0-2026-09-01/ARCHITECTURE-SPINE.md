...
- **Ключ Play Developer API** — секрет в Secret Manager, читается функцией проверки; в репозиторий
  не попадает никогда.
...
- **`maxInstances: 1`** — отгруженное значение сериализует все callable глобально. Для проверки
  чека, которую игрок ждёт на экране, оно не годится: денежные функции получают собственный
  предел, а не общий.
