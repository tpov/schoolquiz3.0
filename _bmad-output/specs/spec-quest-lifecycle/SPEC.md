...
- **CAP-7 — Privilege is granted by a person, never earned by ratings.**
  - **intent:** A qualification level changes only when an admin grants it; ratings feed a separate author-reputation number that confers nothing.
  - **success:** No volume of ratings from any number of accounts changes any privilege gate. Granting is possible only through an admin-gated call, which refuses to act on the caller themselves. Existing farmed levels are zeroed in the same release.
...
- **CAP-12 — Players can report content and block authors.**
  - **intent:** Any player can report a quest or a user from inside the app, and block an author so their content stops appearing.
  - **success:** A report is filed from the app and lands where a moderator can read it; a blocked author's quests stop appearing in the blocking player's lists. Both are reachable within the store-review walkthrough.
...
- **The complaint-judging system the owner described.** The points model must leave room for it; this spec does not build it.
...
- How are qualification points awarded across the whole product? The owner believes they come from complaints judged by a senior moderator or developer; only the rating path is confirmed so far. Under investigation — CAP-7's exact field split depends on the answer.
- Who holds the developer qualification on launch day? Default: the owner alone, granted by hand once.
