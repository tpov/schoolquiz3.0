...
/**
 * Whitelisted Material Icons that the floating-icons background may use. Names are
 * the [androidx.compose.material.icons.Icons.Filled] property names. Lookup is
 * case-insensitive — Firestore can store any casing variant.
 *
 * Adding a new icon: add `Icons.Filled.X to "X"` here, then it becomes valid for
 * any catalog's `iconNames` field in Firestore. R8 keeps icons referenced from
 * this map; unused entries cost nothing in release.
 */
private val materialIconRegistry: Map<String, ImageVector> =
    mapOf(
        // ── Knowledge / school ─────────────────────────────────────────────────
        ...
        // ── Courses / education media ──────────────────────────────────────────
        "OndemandVideo" to Icons.Filled.OndemandVideo,
        "CastForEducation" to Icons.Filled.CastForEducation,
    )

/** Case-insensitive lookup of [materialIconRegistry]. */
private val materialIconRegistryByLowercase: Map<String, ImageVector> =
    materialIconRegistry.entries.associate { (k, v) -> k.lowercase() to v }

/** Resolve an icon name to a Material Icon. Returns null when the name is not whitelisted. */
fun materialIconByName(name: String?): ImageVector? {
    if (name.isNullOrBlank()) return null
    return materialIconRegistryByLowercase[name.trim().lowercase()]
}

/**
 * Resolve a list of icon names to a list of Material Icons. Names that are not in the
 * whitelist are silently skipped — caller decides what to do with the (possibly empty)
 * result.
 */
fun resolveIcons(names: List<String>): List<ImageVector> = names.mapNotNull(::materialIconByName)
