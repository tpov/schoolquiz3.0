...
/**
 * Integration tests for [DefaultRootComponent.onSyncNow] → SyncScheduler + SyncStarted event.
 *
 * Covers SN-phase7-01..03 from docs/features/menu-refactor/plan/phase-07/tests.md.
 * Spec: 07-events.md L3.2 — SyncNow flow.
 *
 * Uses [FakeSyncScheduler] capturing fake — no platform worker classes in presentation tests.
 */
...
class SyncNowFlowIntegrationTest {
    ...
    // -----------------------------------------------------------------------
    // SN-phase7-01
    // GIVEN DefaultRootComponent with FakeSyncScheduler
    // WHEN onSyncNow()
    // THEN scheduler called once
    // -----------------------------------------------------------------------
    @Test
    fun `onSyncNow asks scheduler to enqueue manual sync`() = runTest(testDispatcher) {
        val fakeSyncScheduler = FakeSyncScheduler()
        val component = buildComponent(fakeSyncScheduler = fakeSyncScheduler)

        component.onSyncNow()

        assertEquals(1, fakeSyncScheduler.enqueueManualSyncCalls)
    }
    ...
    // THEN scheduler receives both requests
    // -----------------------------------------------------------------------
    @Test
    fun `two consecutive onSyncNow calls both reach scheduler`() = runTest(testDispatcher) {
        val fakeSyncScheduler = FakeSyncScheduler()
        val component = buildComponent(fakeSyncScheduler = fakeSyncScheduler)
        ...
        assertEquals(2, fakeSyncScheduler.enqueueManualSyncCalls)
    }
}
