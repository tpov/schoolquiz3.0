...
export const getGiftBoxReward = onCall(async (request: CallableRequest) => {
  logger.info('getGiftBoxReward function called');
  ...
  logger.info(`First roll: ${firstRoll}, Second roll: ${secondRoll}`);

  // Check first 5%
  if (firstRoll > 0.05) {
    // Common reward (95% of cases)
    const nolicsAmount = Math.floor(Math.random() * (2000 - 500 + 1)) + 500; // 500-2000
    logger.info(`Common reward: ${nolicsAmount} nolics`);
    return {
      type: 'addNolics',
      amount: nolicsAmount
    } as RewardData;
  ...
  if (secondRoll <= 0.95) {
    ...
    switch (rewardType) {
      case 0: // addNolics
        ...
        logger.info(`Rare reward: ${rareNolics} nolics`);
        return {
          type: 'addNolics',
          amount: rareNolics
        } as RewardData;
      ...
      case 1: // addGold
        ...
        if (goldAmount === 0) {
          ...
          logger.info(`Rare reward (gold was 0): ${alternativeNolics} nolics`);
          return {
            type: 'addNolics',
            amount: alternativeNolics
          } as RewardData;
        }
        logger.info(`Rare reward: ${goldAmount} gold`);
        return {
          type: 'addGold',
          amount: goldAmount
        } as RewardData;
      ...
      case 2: // datePremium
        logger.info('Rare reward: 1 day premium');
        return {
          type: 'datePremium',
  ...
  switch (legendaryType) {
    case 0: // logo
      ...
      logger.info(`Legendary reward: ${randomLogo}`);
      return {
        type: 'logo',
        amount: 1,
        itemName: randomLogo
    ...
    case 1: // trophy
      ...
      if (trophyAmount === 0) {
        ...
        logger.info(`Legendary reward (trophy was 0): ${alternativeGold} gold`);
        return {
          type: 'addGold',
          amount: alternativeGold
        } as RewardData;
      }
      logger.info(`Legendary reward: ${trophyAmount} trophy`);
      return {
        type: 'trophy',
        amount: trophyAmount
      } as RewardData;
    ...
    case 2: // addGold
      ...
      logger.info(`Legendary reward: ${legendaryGold} gold`);
      return {
        type: 'addGold',
    ...
    case 3: // datePremium
      logger.info('Legendary reward: 30 days premium');
      return {
        type: 'datePremium',
    ...
    case 4: // addNolics
      ...
      logger.info(`Legendary reward: ${legendaryNolics} nolics`);
      return {
        type: 'addNolics',
  ...
  logger.info('Fallback reward: 1000 nolics');
  return {
    type: 'addNolics',
    amount: 1000
  } as RewardData;
