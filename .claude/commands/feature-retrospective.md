...
### 1.2.5 Gate Hit-Rate (учёт срабатываний гейтов)

**Цель**: накапливать данные о том, какие гейты реально ловят баги, а какие молчат. Без этих данных пайплайн умеет только расти — каждая ретроспектива добавляет проверки, но ни одна не может обоснованно убрать лишнюю.

Для КАЖДОГО бага заполни строку:

| Bug | Пойман гейтом | Должен был поймать | Стадия обнаружения |
|-----|---------------|--------------------|--------------------|
| #N | `<точное имя гейта/ревьюера/хука>` или `NONE (нашёл пользователь/QA после релиза)` | `<какой гейт по замыслу отвечает за этот класс>` | spec / research / design / plan / per-phase review / cross-phase Codex / smoke / после релиза |

Имена гейтов бери каноническими: `security-reviewer`, `architect-reviewer`, `code-reviewer`, `completeness-reviewer`, `concurrency-reviewer`, `Codex cross-phase`, `grounding Independent Verification`, `Walking Skeleton tests`, `Gate 5 REQUIRES VERIFY`, `Gate 6 Module Direction`, `Gate 7 ADR↔api-contract`, `Gate 8 Multi-Path`, `check-plan-no-code.sh`, `check-plan-paths.sh`, `check-c4-vs-gradle.sh`, `check-api-contract-types.sh`, `Test Deletion Gate`, `smoke ciCheck`, `E2E connectedAndroidTest`, `Spec 5-situation checklist`, `State Matrix validation`.

Затем обнови сводный реестр `docs/features/pipeline-gate-ledger.md` (создай, если его нет) — по одной строке на фичу:

```markdown
| Дата | Фича | Багов | Поймано гейтами (какими) | Пропущено (кто должен был) |
|------|------|-------|--------------------------|----------------------------|
| YYYY-MM-DD | <slug> | N | security-reviewer×1, Codex cross-phase×2 | concurrency-reviewer (async timing), Gate 7 |
```

И обнови в том же файле таблицу накопленных срабатываний:
...
## Фаза 3.5: Обрезка (pruning) — обратный ход храповика

Пайплайн, который умеет только добавлять гейты, со временем задавит себя стоимостью. Эта фаза — единственное место, где разрешено предлагать **удаление**.

Прочитай `docs/features/pipeline-gate-ledger.md` (таблица накопленного hit-rate из Фазы 1.2.5). Кандидат на обрезку — гейт, у которого выполнено ВСЁ:

1. **≥5 фич подряд без единого срабатывания** (по колонке «Фич подряд без срабатывания»);
2. за это время в его зоне ответственности **не было пропущенных багов** (иначе гейт не лишний, а сломан — его надо чинить, а не убирать);
3. он **не является защитой от необратимого** (security, privacy, потеря данных, миграции, деньги — такие гейты не обрезаются НИКОГДА, независимо от статистики);
4. его удаление не ломает другой гейт, который на него ссылается.

Для каждого кандидата покажи пользователю:

```
## Proposed Removal #N: <гейт>

**Файл/секция**: <точное место>
**Статистика**: 0 срабатываний за <N> фич (последнее: <фича, дата> или «никогда»)
**Зона ответственности**: <какой класс проблем он должен ловить>
**Пропуски в этой зоне за период**: <нет / список — тогда это НЕ кандидат>
**Экономия**: <токены/время/шаги за проход>
**Риск удаления**: <что перестанет проверяться и чем это компенсируется>
```

**=WAIT for user approval. Удаление применяется ТОЛЬКО по явному одобрению.=**

Если кандидатов нет — напиши «обрезка не требуется: все гейты срабатывали за последние N фич» и иди дальше. Гейт, срабатывавший хотя бы раз, из реестра не выбывает — статистика копится дальше.
...
- [ ] Ни одна существующая рабочая инструкция не удалена — КРОМЕ обрезок из Фазы 3.5, одобренных пользователем явно
- [ ] Для каждой применённой обрезки в `pipeline-gate-ledger.md` записана строка: дата, что удалено, статистика-основание, кто одобрил
...
- [ ] Заполнена таблица Gate Hit-Rate (Фаза 1.2.5) и обновлён `docs/features/pipeline-gate-ledger.md`
- [ ] Пройдена Фаза 3.5: либо предложены обрезки, либо явно записано «обрезка не требуется»
...
8. **=non-breaking changes=** — новые gates/check добавляются поверх существующих; не перестраивайте рабочие фазы пайплайна. Единственное исключение — обрезка по Фазе 3.5: гейт удаляется только при подтверждённой статистике «0 срабатываний за ≥5 фич», отсутствии пропусков в его зоне, вне защиты от необратимого (security/privacy/данные/миграции/деньги) и с явного одобрения пользователя
11. **=measure before you cut=** — предложение удалить гейт без данных из `pipeline-gate-ledger.md` запрещено. Нет статистики — сначала копи её, потом режь
