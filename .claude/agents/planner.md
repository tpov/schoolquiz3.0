...
- **Stateful field reset**: Если фаза добавляет новые StateFlow, MutableState, in-memory maps или flags в long-lived компонент (ViewModel, Manager, Singleton) — acceptance criteria фазы ОБЯЗАНЫ включать: "новые stateful fields сбрасываются при re-init/cleanup".
