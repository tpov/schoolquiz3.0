...
// Shared builders for the Crypto Smart Money course lesson modules.
// A lesson file exports an array of 40 question objects built with these.
// Mirrors english-tech/_helpers.js, with the crypto-sm id prefix.
...
// Wrap a raw question array into a lesson with stable crypto-sm ids.
function buildLesson(s, t, l, lessonId, lessonTitle, questions) {
  ...
  const wrapped = questions.map((q, order) => {
    ...
    const id = `qsb-courses-crypto-sm-${s}-${t}-${l}-${TYPE_PREFIX[q.type]}-${DIFF_PREFIX[q.difficulty]}-${counters[key]}`;
