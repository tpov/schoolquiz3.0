...
// Shared builders for the German A0-C2 course lesson modules.
// A lesson file exports an array of 40 question objects built with these.
// Mirrors english-tech/_helpers.js, with the german id prefix.
...
// Wrap a raw question array into a lesson with stable german ids.
function buildLesson(s, t, l, lessonId, lessonTitle, questions) {
  ...
  const wrapped = questions.map((q, order) => {
    ...
    const id = `qsb-courses-german-${s}-${t}-${l}-${TYPE_PREFIX[q.type]}-${DIFF_PREFIX[q.difficulty]}-${counters[key]}`;
