...
// Shared builders for the school math course lesson modules.
// A lesson file exports an array of 40 question objects built with these.
// Mirrors courses/business/_helpers.js, with the school-math-full id prefix
...
function buildLesson(s, t, l, lessonId, lessonTitle, questions) {
  ...
  const wrapped = questions.map((q, order) => {
    ...
    const id = `qsb-school-math-full-${s}-${t}-${l}-${TYPE_PREFIX[q.type]}-${DIFF_PREFIX[q.difficulty]}-${counters[key]}`;
