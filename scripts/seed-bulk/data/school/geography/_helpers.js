...
// Shared builders for the school geography course lesson modules.
// A lesson file exports an array of 40 question objects built with these.
// Mirrors courses/business/_helpers.js, with the school-geography id prefix
// (the legacy 8-lesson pilot occupies the plain `school-math` prefix).
...
// Wrap a raw question array into a lesson with stable school-math-full ids.
function buildLesson(s, t, l, lessonId, lessonTitle, questions) {
  ...
  const wrapped = questions.map((q, order) => {
    ...
    const id = `qsb-school-geography-${s}-${t}-${l}-${TYPE_PREFIX[q.type]}-${DIFF_PREFIX[q.difficulty]}-${counters[key]}`;
