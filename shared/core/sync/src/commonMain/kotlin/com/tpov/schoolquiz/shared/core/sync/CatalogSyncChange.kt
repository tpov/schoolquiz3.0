...
interface CatalogSyncChangeRemoteDataSource {
     ...
     *
     * Она же отдаёт [SyncChangePage.unreadable] нулём, потому что считать нечего: сюда приходит
     * уже разобранное. Реализация, которая разбирает документы сама, обязана назвать это число —
     * терпимость к трём формам снимается только когда видно, что пропусков не осталось (AD-11).
     */
