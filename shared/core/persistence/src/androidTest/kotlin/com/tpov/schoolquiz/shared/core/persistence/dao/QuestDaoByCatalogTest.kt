...
class QuestDaoByCatalogTest {
    ...
    @Test
    fun `quests with matching catalogId returned`() = runTest {
        insertCatalog("catA")
        insertCatalog("catB")
    ...
    @Test
    fun `archived quests excluded from result`() = runTest {
        insertCatalog("catA")
    ...
    @Test
    fun `empty table emits empty list, no error`() = runTest {
    ...
    @Test
    fun `insert new quest re-emits via Flow`() = runTest {
        insertCatalog("catA")
    ...
    @Test
    fun `delimiter wrapped LIKE exact match includes quest with exact shelf name`() = runTest {
        insertCatalog("catA")
        dao.insertOrReplace(
    ...
    @Test
    fun `delimiter wrapped LIKE exact match excludes quest whose visibleOn contains shelf as substring`() = runTest {
        insertCatalog("catA")
        dao.insertOrReplace(
    ...
    @Test
    fun `quest with empty visibleOn does not crash and is not returned`() = runTest {
        insertCatalog("catA")
        dao.insertOrReplace(
